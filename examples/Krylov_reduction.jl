using FlexibleMultibody
using Gridap, GridapGmsh
using Arpack
using LinearAlgebra
using SparseArrays
using Printf
using Plots

<<<<<<< HEAD
# ==============================================================================
#  МЕТОД КРЫЛОВСКИХ ПОДПРОСТРАНСТВ
#
#  Алгоритм:
#  1. Берём нагрузку b (единичная сила в DOF_EXCITE)
#  2. Строим базис Крылова через процесс Арнольди:
#       v₁ = K⁻¹b  (статический отклик)
#       vₙ = K⁻¹M·vₙ₋₁  (динамические поправки)
#     Каждый вектор ортогонализируем (Грам-Шмидт) чтобы базис был устойчив
#  3. Редуцируем: K_kr = Vᵀ·K·V,  M_kr = Vᵀ·M·V
#  4. Решаем задачу на собственные значения редуцированной системы
#  5. Сравниваем частоты и FRF с полной моделью
# ==============================================================================

# ==============================================================================
#  Шаг 1: Строим модель
# ==============================================================================
=======
# Редукция методом Крылова: H(ω) раскладывается в ряд по ω² около σ=0
# Базис строится из статических откликов на нагрузки в граничных DOF
# + инерционные поправки K⁻¹Mv для каждого вектора

>>>>>>> eb17070f8145b2740b9fff57783cde3bb9a30693
mat = LinearElasticMaterial(2.1e11, 0.3, 7.85e3)

rbe_info = [
    RBEInfo("left_hole",  VectorValue(0.0, 0.0, 0.0)),
    RBEInfo("right_hole", VectorValue(0.7, 0.0, 0.0)),
]

println("Сборка модели...")
fl_rod  = FlexibleComponent("link_mesh.msh", mat; rbe_info)
K       = fl_rod.mK
M       = fl_rod.mM
n_total = size(K, 1)
println("Всего DOF: $n_total")

<<<<<<< HEAD
# ==============================================================================
#  Шаг 2: DOF возбуждения
#  Нагрузка b — единичная сила в первом DOF
#  (можно менять на любой другой DOF)
# ==============================================================================
=======
# DOF входа и выхода для FRF
>>>>>>> eb17070f8145b2740b9fff57783cde3bb9a30693
DOF_EXCITE  = 1
DOF_RESPOND = 1

b = zeros(n_total)
b[DOF_EXCITE] = 1.0

<<<<<<< HEAD
# ==============================================================================
#  Шаг 3: Построение базиса Крылова через процесс Арнольди
#
#  Строим подпространство:
#  K(K,M,b,m) = span{ K⁻¹b,  K⁻¹M·K⁻¹b,  (K⁻¹M)²·K⁻¹b, ... }
#
#  БЛОЧНЫЙ КРЫЛОВ — стартуем из ВСЕХ граничных DOF одновременно
#  Это аналог того что делает CB: захватывает все моды через все интерфейсы
#  Для каждого граничного DOF строим свою цепочку векторов
# ==============================================================================
KRYLOV_PER_DOF = 3    # векторов на каждый граничный DOF
                       # итого: 12 граничных × 3 = 36 векторов

println("\nПостроение блочного базиса Крылова...")
println("  Граничных DOF: $n_b  ×  $KRYLOV_PER_DOF векторов = $(n_b * KRYLOV_PER_DOF) векторов")
=======
# Граничные DOF — два интерфейса RBE2 по 6 DOF каждый
boundary_dofs = Int[]
for (_, iface) in fl_rod.interfaces
    append!(boundary_dofs, collect(iface.dofs))
end
sort!(boundary_dofs)
interior_dofs = setdiff(1:n_total, boundary_dofs)
n_b = length(boundary_dofs)
n_i = length(interior_dofs)

# Число векторов Крылова на каждый граничный DOF:
# k=1 → только статика (= моды ограничений CB)
# k=2 → + инерция первого порядка
# k=3 → + инерция второго порядка
KRYLOV_PER_DOF = 3

println("\nПостроение базиса Крылова ($n_b граничных DOF × $KRYLOV_PER_DOF векторов = $(n_b*KRYLOV_PER_DOF) итого)...")
>>>>>>> eb17070f8145b2740b9fff57783cde3bb9a30693

K_dense = Matrix(K)
M_dense = Matrix(M)

<<<<<<< HEAD
println("  Факторизация K...")
K_fact = factorize(K_dense)

# Собираем все базисные векторы сюда
cols = Vector{Vector{Float64}}()

# Для каждого граничного DOF строим цепочку Крылова
for dof in boundary_dofs
    # Стартовый вектор — единичная нагрузка в этом DOF
    b_dof = zeros(n_total)
    b_dof[dof] = 1.0

    v = K_fact \ b_dof   # статический отклик K⁻¹·b

    for k in 1:KRYLOV_PER_DOF
        # Ортогонализация Грама-Шмидта относительно всех уже добавленных векторов
=======
# Факторизуем один раз — используем для всех итераций
K_fact = factorize(K_dense)

cols = Vector{Vector{Float64}}()

for dof in boundary_dofs
    b_dof = zeros(n_total)
    b_dof[dof] = 1.0

    v = K_fact \ b_dof  # v₁ = K⁻¹b, статический отклик

    for k in 1:KRYLOV_PER_DOF
        # Грам-Шмидт: убираем компоненты вдоль уже добавленных векторов
>>>>>>> eb17070f8145b2740b9fff57783cde3bb9a30693
        for u in cols
            v = v - dot(v, u) * u
        end

        nv = norm(v)
<<<<<<< HEAD
        nv < 1e-12 && break   # вектор линейно зависим — пропускаем
=======
        nv < 1e-12 && break  # вектор линейно зависим — подпространство исчерпано
>>>>>>> eb17070f8145b2740b9fff57783cde3bb9a30693

        v = v / nv
        push!(cols, copy(v))

<<<<<<< HEAD
        # Следующий вектор в цепочке: K⁻¹·M·v
        k < KRYLOV_PER_DOF && (v = K_fact \ (M_dense * v))
=======
        k < KRYLOV_PER_DOF && (v = K_fact \ (M_dense * v))  # следующий: K⁻¹Mv
>>>>>>> eb17070f8145b2740b9fff57783cde3bb9a30693
    end
end

V   = hcat(cols...)
n_V = size(V, 2)
<<<<<<< HEAD
println("  Итоговый размер базиса: $n_V векторов")

ortho_err = norm(V' * V - I)
println("  Ошибка ортогональности ‖VᵀV - I‖ = $(round(ortho_err, digits=8))")

# ==============================================================================
#  Шаг 4: Проецируем матрицы на подпространство Крылова
#
#  K_kr = Vᵀ · K · V   (NUM_KRYLOV × NUM_KRYLOV)
#  M_kr = Vᵀ · M · V   (NUM_KRYLOV × NUM_KRYLOV)
#
#  Это гарантирует что редуцированная система точно воспроизводит
#  отклик полной системы в направлениях базиса V
# ==============================================================================
println("\nПроекция матриц на подпространство Крылова...")
=======
println("  Размер базиса: $n_V векторов")
println("  Ошибка ортогональности ‖VᵀV - I‖ = $(round(norm(V'*V - I), digits=8))")

# Проекция: K_red = VᵀKV, M_red = VᵀMV
println("\nРедукция матриц...")
>>>>>>> eb17070f8145b2740b9fff57783cde3bb9a30693
K_kr = Symmetric(V' * K_dense * V)
M_kr = Symmetric(V' * M_dense * V)

n_red = size(K_kr, 1)
<<<<<<< HEAD
println("Размер редуцированной системы: $n_red × $n_red")
println("Степень редукции: $(round(100*(1 - n_red/n_total), digits=1))%")

# ==============================================================================
#  Шаг 5: Собственные значения полной модели (для сравнения)
# ==============================================================================
NUM_MODES = 20
println("\nВычисление мод полной модели...")
=======
println("Размер редуцированной системы: $n_red DOF  (редукция $(round(100*(1-n_red/n_total), digits=1))%)")

# Собственные моды полной модели для сравнения
NUM_MODES = 10
println("\nМоды полной модели...")
>>>>>>> eb17070f8145b2740b9fff57783cde3bb9a30693

λ_fb, Φ_fb = eigs(M, K; nev=NUM_MODES+6, which=:LM)
order_fb   = sortperm(real.(λ_fb), rev=true)
λ_fb       = real.(λ_fb)[order_fb]
Φ_fb       = real.(Φ_fb)[:, order_fb]

ω2_fb   = 1.0 ./ abs.(λ_fb)
<<<<<<< HEAD
mask_fb = ω2_fb .> (2π)^2
ω2_fb   = ω2_fb[mask_fb]
Φ_fb    = Φ_fb[:, mask_fb]

# Нормировка
=======
mask_fb = ω2_fb .> (2π)^2  # убираем жёсткотельные моды
ω2_fb   = ω2_fb[mask_fb]
Φ_fb    = Φ_fb[:, mask_fb]

# Нормировка по массе: φᵀMφ = 1
>>>>>>> eb17070f8145b2740b9fff57783cde3bb9a30693
for i in axes(Φ_fb, 2)
    mn = sqrt(abs(Φ_fb[:,i]' * M_dense * Φ_fb[:,i]))
    mn > 1e-12 && (Φ_fb[:,i] ./= mn)
end

freq_full = sqrt.(ω2_fb) ./ (2π)

<<<<<<< HEAD
# ==============================================================================
#  Шаг 6: Собственные значения редуцированной модели Крылова
# ==============================================================================
println("Вычисление мод редуцированной модели Крылова...")
=======
# Собственные моды редуцированной модели
println("Моды редуцированной модели...")
>>>>>>> eb17070f8145b2740b9fff57783cde3bb9a30693
λ_kr, Φ_kr = eigen(K_kr, M_kr)
order_kr   = sortperm(real.(λ_kr))
λ_kr       = real.(λ_kr)[order_kr]
Φ_kr       = real.(Φ_kr)[:, order_kr]

ω2_kr   = λ_kr
mask_kr = ω2_kr .> (2π)^2
ω2_kr   = ω2_kr[mask_kr]
Φ_kr    = Φ_kr[:, mask_kr]

freq_kr = sqrt.(ω2_kr) ./ (2π)

<<<<<<< HEAD
# Переводим моды в физическое пространство: Φ_phys = V · Φ_kr
Φ_kr_phys = V * Φ_kr

# Нормировка
=======
# Возврат мод в физическое пространство: φ_phys = V·φ_kr
Φ_kr_phys = V * Φ_kr

>>>>>>> eb17070f8145b2740b9fff57783cde3bb9a30693
for i in axes(Φ_kr_phys, 2)
    mn = sqrt(abs(Φ_kr_phys[:,i]' * M_dense * Φ_kr_phys[:,i]))
    mn > 1e-12 && (Φ_kr_phys[:,i] ./= mn)
end

<<<<<<< HEAD
# ==============================================================================
#  Шаг 7: Таблица сравнения частот
# ==============================================================================
n_cmp = min(length(freq_full), length(freq_kr), NUM_MODES)
println("\n" * "="^62)
println("    СРАВНЕНИЕ ЧАСТОТ (Гц) — Крыловское подпространство")
=======
# Сравнение частот
n_cmp = min(length(freq_full), length(freq_kr), NUM_MODES)
println("\n" * "="^62)
println("        СРАВНЕНИЕ ЧАСТОТ (Гц)")
>>>>>>> eb17070f8145b2740b9fff57783cde3bb9a30693
println("="^62)
@printf("%-6s  %-16s  %-16s  %-10s\n", "Мода", "Полная модель", "Krylov", "Ошибка %")
println("-"^62)
for i in 1:n_cmp
    f1  = freq_full[i]
    f2  = freq_kr[i]
    err = abs(f1 - f2) / f1 * 100
    @printf("%-6d  %-16.2f  %-16.2f  %-10.4f\n", i, f1, f2, err)
end
println("="^62)
println("Редуцированная модель: $n_red DOF вместо $n_total DOF")

<<<<<<< HEAD
# ==============================================================================
#  Шаг 8: FRF — полная модель vs Крылов
#  H(ω) = Σ φᵢ(in)·φᵢ(out) / (ωᵢ² - ω² + 2i·ζ·ωᵢ·ω)
# ==============================================================================
=======
# FRF методом модальной суперпозиции
>>>>>>> eb17070f8145b2740b9fff57783cde3bb9a30693
println("\nПостроение FRF...")

ζ     = 0.02
f_min = 50.0
f_max = freq_full[end] * 1.3
freqs = range(f_min, f_max, length=3000)
ωs    = 2π .* collect(freqs)

<<<<<<< HEAD
# Полная модель
=======
>>>>>>> eb17070f8145b2740b9fff57783cde3bb9a30693
H_full = zeros(ComplexF64, length(ωs))
for (k, ω) in enumerate(ωs)
    for i in eachindex(ω2_fb)
        ωi = sqrt(ω2_fb[i])
        H_full[k] += Φ_fb[DOF_EXCITE, i] * Φ_fb[DOF_RESPOND, i] /
                     (ω2_fb[i] - ω^2 + 2im*ζ*ωi*ω)
    end
end

<<<<<<< HEAD
# Крыловская модель
=======
>>>>>>> eb17070f8145b2740b9fff57783cde3bb9a30693
H_kr = zeros(ComplexF64, length(ωs))
for (k, ω) in enumerate(ωs)
    for i in eachindex(ω2_kr)
        ωi = sqrt(ω2_kr[i])
        H_kr[k] += Φ_kr_phys[DOF_EXCITE, i] * Φ_kr_phys[DOF_RESPOND, i] /
                   (ω2_kr[i] - ω^2 + 2im*ζ*ωi*ω)
    end
end

<<<<<<< HEAD
# ==============================================================================
#  График
# ==============================================================================
=======
>>>>>>> eb17070f8145b2740b9fff57783cde3bb9a30693
H_full_dB = 20 .* log10.(abs.(H_full) .+ 1e-40)
H_kr_dB   = 20 .* log10.(abs.(H_kr)   .+ 1e-40)

p = plot(
    freqs, H_full_dB,
    label     = "Полная модель",
    color     = :blue,
    linewidth = 2,
    xlabel    = "Частота (Гц)",
    ylabel    = "Амплитуда |H(ω)| (дБ)",
    title     = "FRF: Krylov ($n_red векторов) vs Полная модель  |  DOF $DOF_EXCITE → $DOF_RESPOND",
    legend    = :topright,
    grid      = true,
    size      = (1100, 550)
)

plot!(p,
    freqs, H_kr_dB,
    label     = "Krylov ($n_red векторов)",
    color     = :green,
    linewidth = 1.5,
    linestyle = :dash
)

for f in freq_full
    vline!(p, [f], color=:gray, alpha=0.4, linewidth=1, label="")
end

savefig(p, "bode_krylov.png")
println("График сохранён: bode_krylov.png")
<<<<<<< HEAD
display(p)
=======
display(p)
>>>>>>> eb17070f8145b2740b9fff57783cde3bb9a30693
